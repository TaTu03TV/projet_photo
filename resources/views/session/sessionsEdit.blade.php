...
<h1 class="text-center stylish-title">Modifier la session </h1>
...
<div class="container">
    ...
    <form action="{{ route('sessions.update', $session->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="date" class="form-control" id="date" name="date" value="{{ $session->session_date }}" required>

            <label for="location">Lieu</label>
            <input type="text" class="form-control" id="location" name="location" value="{{ $session->location }}" required>

            <label for="description">Description</label>
            <textarea class="form-control" id="description" name="description" required>{{ $session->description }}</textarea>  
